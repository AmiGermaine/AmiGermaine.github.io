// These functions open and close the conctact form 
function openForm() {
...
function showSlides(n) {
    var slides = document.getElementsByClassName("mySlides"); // This takes all elements with the class name
    // "mySlides" and stores them in the variable array "slides"
    ...
    if (n < 1) {slideIndex = slides.length}; // If n (the number passed into the function) is less then 1,
    //the slideIndex is set to the length of the array "slides"
    for (i = 0; i < slides.length; i++) {
        slides[i].style.display = "none"; // This for loop takes each item in the array "slides" 
        //and sets the display to none   
    }
    for (i = 0; i < dots.length; i++) {
        dots[i].className = dots[i].className.replace(" active", ""); // This for loop takes each item in the array 
